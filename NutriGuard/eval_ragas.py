...
async def eval_faithfulness(answer: str, context: str) -> dict:
    prompt = FAITHFULNESS_PROMPT.format(
        context=context[:2500] if context else "",
...
async def main():
    ...
    for item in SAMPLE_QUERIES:
        ...
        if len(filtered) < 2:  # at least 2 docs
            filtered = scored[:3]
        else:
            filtered = filtered[:3]

        # 2a.5: LLM relevance filter — 快速判断每个 chunk 是否相关
        rel_filter_prompt = f"""用户问题：{query}\n\n判断以下 3 个文本片段是否与问题相关。每个片段输出 0(无关) 或 1(相关)，用逗号分隔。只输出三个数字，如：1,0,1"""
        chunk_snippets = "\n---\n".join([d.page_content[:200] for d, _ in scored[:3]])
        try:
            ...
            if len(rel_scores) == 3:
                # Filter out irrelevant docs — no fallback, trust the filter
                filtered_docs = [(d, s) for (d, s), r in zip(scored[:3], rel_scores) if r == 1]
                if filtered_docs:
                    scored = filtered_docs
                print(f"    LLM filter: {rel_scores} → kept {len(scored)} docs")
        except Exception:
            pass  # filter fails → keep all
