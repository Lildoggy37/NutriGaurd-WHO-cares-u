...
**一句话定位**：独立设计的多智能体 AI 膳食管家，基于 LangGraph Supervisor 模式协调 8 个 Agent 节点，通过 MCP 协议接入 9 个工具，支持多模态视觉（拍照识食+冰箱食谱），覆盖 RAG 检索、饮食记录、合规审查、记忆管理全链路。

**技术栈**：`Python` `LangGraph` `LangChain` `FastAPI` `Qdrant` `BGE/Reranker` `Redis Stack` `SQLite` `MCP` `qwen-plus` `qwen-vl-plus` `Next.js 14` `Docker`
...
| 工具调用缺乏标准 | MCP 协议封装 9 个工具，按职能分派给不同 Agent |
| 纯文本无法处理食物照片 | 新增 vision_expert 节点(qwen-vl-plus) + 前端拍照上传 + generate_recipe MCP 工具 |
...
└──────────────┬──────────────────────────┘
    ...
    │     ┌────────┼────────┬──────────┬───────┐   │
    │     ▼        ▼        ▼          ▼       ▼   │
    │  rag      action    slot     vision    END  │
    │  expert   expert   filler   expert          │
    │     │        │                 │            │
    │     ▼        ▼                 │            │
    │  rag       memory              │            │
...
```
用户: [拍冰箱照片] + "帮我规划晚餐"
  → FastAPI: ChatRequest { query, image_base64 }
    → 构造 multimodal HumanMessage([text, image_url])
  → preprocess → supervisor (检测到 image_url) → route: vision_expert
    → vision_expert (qwen-vl-plus):
       分析图片 → {"scene":"fridge","ingredients":["鸡胸肉","西兰花","胡萝卜"]}
    → supervisor (二次路由, scene=fridge) → route: action_expert
    → action_expert LLM 决定: 调用 generate_recipe("鸡胸肉,西兰花,胡萝卜", user_id)
      → generate_recipe MCP工具:
         ① lookup_food() ×3 (SQLite) → 营养数据
         ② get_health_profile() → 糖尿病
         ③ calculate_daily_target() → 剩余1368kcal
         ④ perform_rag_search("糖尿病 禁忌") → RAG检索
         ⑤ GI过滤 → safe[]/warning[]
         ⑥ 返回「食谱生成指令」(自然语言, 含8条生成要求)
    → action_expert LLM 接收 ToolMessage → 创意合成最终食谱文本
    → SSE streaming → 前端展示: "推荐蒜蓉西兰花炒鸡胸肉..."
```
...
### 3.5 视觉模型：qwen-vl-plus (DashScope)

| 标准 | qwen-vl-plus | qwen-vl-max | 百度菜品识别 |
|------|-------------|------------|------------|
| 食物识别 | 通用VL，中文食物好 | 更强（小字/摆盘） | 专用分类器，更准 |
| 单次成本 | ~￥0.003 | ~￥0.006 | ~￥0.001 |
| 接入难度 | 零（同 DashScope Key） | 零 | 中（新 SDK+Key） |
| 对话能力 | 原生多轮 | 原生多轮 | 无（仅返回标签） |
| 分量估算 | 视觉估算 | 视觉估算 | 无 |

**选 qwen-vl-plus 原因**：项目已用 DashScope qwen-plus，同一 `base_url`/API Key，仅改 model name + 构造 multimodal message。图片在前端压缩到 1024px JPEG Q=0.7（~100KB），通过 base64 经 SSE 传输。分量估算置信度不稳定时由 slot_filler 追问补充。
...
选 Qdrant 的核心原因：**磁盘模式 + Hybrid Search 原生支持**。开发阶段不需要独立部署，一行 `path="./qdrant_storage"` 即可持久化 17,749 个 chunk 的向量索引（209MB）。Hybrid Search 的 Dense+Sparse 双路召回是 Qdrant 的原生 API，不需要额外实现。
...
| 指标 | 值 | 含义 |
|------|-----|------|
| Recall@3 | 88% | 100 条中 88 条在 Top-3 找到目标内容 |
| MRR | 0.817 | 正确答案平均排在第 1.2 位 |
| RAGAS Faithfulness | 1.000 | 24条分层抽样, 回答声明 100% 基于检索证据 |
| RAGAS Answer Relevancy | 0.622 | 回答与问题的语义相关性 |
| RAGAS Context Recall | 0.735 | 检索覆盖了 73.5% 的相关信息 |
...
| 阶段 | Faith. | Rel. | Prec. | Recall | 变化说明 |
|------|--------|------|-------|--------|---------|
| v1: 5条手选, 94 chunks | 1.000 | 0.660 | 0.667 | 0.733 | 初版：样本太少, 都选语料覆盖好的, "太假" |
| v2: 24条分层, 94 chunks | 0.854 | 0.511 | 0.458 | 0.654 | 暴露真实覆盖差距（新增疾病检索不到证据） |
| **v3: 24条分层, 17,749 chunks** | **1.000** | **0.622** | **0.508** | **0.735** | **语料240x + Top-5证据池 + "不猜测"prompt加固** |

**v3 的改动**：
1. 语料从 94 chunks 扩到 17,749 chunks（8个开源数据集）→ 检索覆盖大幅提升
2. Retriever k=10→20, Reranker Top-3→5 → 给 LLM 更宽裕的证据池
3. RAG Expert system prompt 加固："证据不足时明确说无法确定，不要猜测"

**核心结论**：Faithfulness 从 0.854 回升到 1.000 不是"倒退回去"——v1 的 1.000 是样本太少导致的假象，v3 的 1.000 是在 24 条分层抽样 + 17K chunks 基础上通过语料扩充和参数调优达到的真实结果。四项指标全面超越 v2 和 v1（Precision 除外，因 17K 文档中语义相似但实际无关的候选更多）。
...
| 7a | 语料规模化扩充 28K→3.5M | 74 chunks 仅占 21% 上下文→RAG 形同虚设 | **17,749 chunks, Qdrant 800KB→209MB, 8 个开源数据集导入, 零 API 消耗** | **+240x** |
...
| 12 | RAGAS 5→24 条 | Faithfulness=1.0 太假 | 0.854 (暴露真实覆盖差距) |
| 13 | 语料规模化扩充 28K→3.5M | 74 chunks 仅占 21% 上下文→RAG 形同虚设 | 17,749 chunks, Qdrant 800KB→209MB, 8 个开源数据集 | +240x |
| 14 | RAG 参数调优 + RAGAS 重测 | Faithfulness 0.854 在医疗场景偏低 | Retriever k=20, Top-5, "不猜测"prompt, Faithfulness=1.000 | 四项全维提升 |
...
### Q23: "RAG 语料是哪里来的？怎么保证可信度？多大量？"

> "语料来自两个渠道：基础部分汇编自国家级指南（中国营养学会《膳食指南(2025)》、国家卫健委痛风/高血压/糖尿病食养指南(2024)等）；大规模扩充来自 **8 个开源数据集**——GitHub 上的中国食物成分表(1,677种食物+GI)和食材食疗数据库(3,092种)，HuggingFace 上的 DoctorFLAN(中文疾病饮食指导)、华佗百科医疗问答、CMMLU 营养学题库、MM-Food-100K 中餐菜品营养数据等。**总计 17,749 chunks / 3.5M 字符**——这个量级下 RAG 检索才是刚需而非装饰。可信度靠两层：数据源权威性（国家级指南/学术数据集）+ Reflection 审查（LLM 回答后校验是否基于检索证据）。将来可按 section_id+SHA256 追溯每个 chunk 的来源数据集。"
...
> "SQLite `nutriguard.db` ~100KB（41 种基础食物 + 用户画像 + 长期记忆 + 餐食记录 + 12,218 条 section 元数据）。Qdrant 磁盘模式 **209.3 MB**——17,749 个 chunk 的 BGE 1024 维 Dense 向量 + BM25 Sparse 向量 + 元数据索引, 从 28K 字语料扩到 3.5M 字后向量存储增长了 267 倍。单用户场景的 SQLite 增速不变（每天几条记录），Qdrant 是静态索引（只在语料更新时增量重索引）。"
...
### Q33: "RAGAS Faithfulness 从 1.0 降到 0.854 又回到 1.0，怎么回事？"

> "这个波动正好展示了评测方法论的三次迭代：
>
> **v1 (5条手选, 94 chunks): Faithfulness=1.000** — 假的。5 条都是语料覆盖好的常见问题，样本太少让指标看起来完美。
>
> **v2 (24条分层抽样, 94 chunks): Faithfulness=0.854** — 暴露了真相。分层抽样包含了语料没覆盖的疾病（肝硬化、甲亢等），检索找不到证据，LLM 只能编造，Faithfulness 应声下降。
>
> **v3 (24条分层抽样, 17,749 chunks): Faithfulness=1.000** — 真的。三个关键改动：(1)语料从 74 chunks 扩到 17,749 chunks（8个开源数据集，GitHub+HuggingFace），覆盖了 v2 暴露出的缺口；(2)Retriever k=10→20, Reranker Top-3→5，给 LLM 更宽裕的证据池；(3)RAG Expert system prompt 加固'证据不足时明确说无法确定，不要猜测'。四项 RAGAS 指标全部提升，Answer Relevancy 从 0.511 跳到 0.622，Context Recall 从 0.654 跳到 0.735。
>
> 面试官问这个问题，我会展示这张表并解释：第一版 1.000 是样本不足的假象，第二版 0.854 是分层抽样的真实暴露，第三版 1.000 是通过语料扩充和参数调优真正实现的——这恰好说明了我知道怎么用评测数据驱动系统优化，而不是满足于一个好看的数字。""
...
### Q40: "多模态是怎么做的？拍照识食和冰箱食谱两个场景的技术链路有什么不同？"

> "我新增了一个 **vision_expert 节点**（第8个节点，使用 qwen-vl-plus 视觉模型）和一个 **generate_recipe MCP 工具**（第9个工具）。两个场景共享同样的视觉识别入口，但下游路由不同。
>
> **拍照记食链路**：用户拍照 → 前端 compressImage(1024px+JPEG Q=0.7, ~100KB) → base64 通过 SSE JSON body 传输 → api_server 构造 multimodal HumanMessage([text, image_url]) → supervisor 检测 image_url 自动路由 vision_expert → qwen-vl-plus 输出 `{\"scene\":\"meal\",\"items\":[{\"name\":\"红烧肉\",\"amount_g\":200,\"confidence\":\"low\"}],\"uncertain_items\":[\"红烧肉分量看不清\"]}` → supervisor 发现 confidence=low 路由 slot_filler → slot_filler 追问\"红烧肉大概是200g吗？\" → 用户确认后 action_expert 调用 log_user_meal + calculate_daily_calories → 自动记录。
>
> **冰箱食谱链路**：用户拍照 → 同上到 vision_expert → 输出 `{\"scene\":\"fridge\",\"ingredients\":[\"鸡胸肉\",\"西兰花\",\"胡萝卜\",\"鸡蛋\"]}` → supervisor 路由 action_expert → LLM 决定调用 generate_recipe(ingredients, user_id) → 工具内部查 SQLite(lookup_food 获取每100g营养) + RAG(perform_rag_search 查疾病禁忌) + 热量计算(calculate_daily_target) + GI过滤 → 返回「食谱生成指令」(自然语言格式，含8条明确的生成要求，如\"设计2-3个菜品\"\"遵守疾病禁忌\"\"标注热量\") → action_expert 的 qwen-plus LLM 接收 ToolMessage 后创意合成最终食谱 → 流式返回。
>
> **关键设计决策**：generate_recipe 工具只做数据聚合（SQLite + RAG + 计算），不调 LLM。创意合成（菜品设计、烹饪方式、营养建议）留给 action_expert 的 LLM 完成。这样 MCP 工具保持 LLM-free 的架构一致性——所有 9 个工具都不依赖 LLM，LLM 能力集中在 graph_brain.py 的 8 个节点中。视觉模型选 qwen-vl-plus 而非百度专用食物 API，因为项目已用 DashScope qwen-plus，同一 API Key/Base URL 零额外接入成本，且 VL 模型的对话能力让用户可以追问'这个热量高吗？适合糖尿病患者吗？'——专用分类器只能返回标签，无法支持多轮对话。"
...
| 注入 | Prompt 注入防御已实现 | 10 个注入模式 Preprocess 正则拦截 + System Prompt 加固 | ✅ |
| 超大输入 | 单条消息无长度限制 | 5 万字体检报告可能导致 MemorySaver OOM | 5000 字符截断 + 文件异步解析 |
| 越权 | JWT 认证已实现 | enforce_user_id 强制覆盖, 3个工具接线 | ✅ |
...
| 视觉精度 | VL 模型食物分量估算置信度不稳定 | 克数不准确导致热量偏差 | slot_filler 追问 + 多角度拍照 |
...
>
> AI 方向上，我独立设计和实现了一个叫 NutriGuard 的多智能体 AI 膳食管家。核心架构基于 LangGraph Supervisor 模式，协调了 8 个 Agent 节点，通过 MCP 协议标准化接入了 9 个工具。技术上主要解决了三个问题：一是单 Agent 长链路不可靠，我用 Supervisor 做了意图分发加 Harness 框架做统一重试和降级；二是上下文容易漂移，我用三层记忆架构（Redis 工作记忆 + LangGraph 短期 + SQLite 长期）加 token 阈值压缩来管理；三是纯文本无法处理食物照片，我引入了 qwen-vl-plus 视觉模型做拍照识食和冰箱食谱规划，通过 vision_expert 节点分析图片后自动对接下游的饮食记录工具或食谱生成工具。评估方面，我自建了 100 条 RAG 评测集加 RAGAS 生成质量评估，经历了 6 轮优化把 Context Precision 从 0.40 提到了 0.67。项目有 14 个 pytest 单元测试，Docker 一键部署。
>
