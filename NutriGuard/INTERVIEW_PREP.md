...
> "语料汇编自三个国家级指南：中国营养学会《中国居民膳食指南(2025)》、国家卫健委《成人高尿酸血症与痛风食养指南(2024)》、《中国2型糖尿病防治指南(2024)》。文件头标注了来源，65K 字手写整理。可信度靠两层保证：数据源权威（国家级机构指南）+ Reflection 审查（LLM 生成回答后校验是否基于检索证据，幻觉自动拦截）。诚实说——这是我个人的手工汇编而非官方 PDF 直接解析，生产环境需要自动解析原文并给每段结果带来源引用。RAGAS Faithfulness 1.0 证明当前版本没有超出检索证据的幻觉。"
...
> "极小。SQLite `nutriguard.db` ~100KB（41 种食物 + 用户画像 + 长期记忆 + 餐食记录）。Qdrant 内存模式 ~800KB（80+ 个 chunk 的 Dense + Sparse 向量）。单用户场景每天新增几条记录，增速可忽略。"
